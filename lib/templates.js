...
function getRaw() {
  ...
  if (files) {
    Y.Object.keys(files).forEach(function(dir) {
      files[dir].forEach(function(file) {
        ...
        if (file.slice(0, 1) === '.' || (
            ext !== '.handlebars' && ext !== '.partial' && ext !== '.jsx')) {
          return;
        }
  ...
  cache.react = Y.merge(react);
  return {raw: raw, partials: partials, react: react};
}
...
function getPrecompiled() {
  ...
  Y.Object.each(raw.react, function(template, name) {
    try {
      precompiled.react['react-' + name] = "function() {   if (this.props.name === 'featured') { this.props.name = 'yippeee'; }return (" +
        ReactTools.transform("/** @jsx React.DOM */" + template) +
        ');}';
    } catch (x) {
      console.error('Error compiling jsx: ', name, x.message);
    }
  })
...
function prepareTemplate() {
  ...
  Y.Object.each(precompiled.react, function(template, name) {
    react.push({
      name: name,
      template: template
    });
